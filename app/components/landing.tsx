...
const Landing: React.FC = () => {
    ...
    useEffect(() => {
      const fetchCompetencias = async () => {
        ...
        const competenciasQuery = query(competenciasCollection, orderBy('date', 'desc'));
        const competenciasSnapshot = await getDocs(competenciasQuery);
        const competenciasList = competenciasSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
        }) as Competencia);
      ...
      const fetchExperiencias = async () => {
        ...
        const experienciasQuery = query(experienciasCollection, orderBy('date', 'desc'));
        const experienciasSnapshot = await getDocs(experienciasQuery);
        const experienciasList = experienciasSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
        }) as Experience);
      ...
      <main id='Gil.h'>
        ...
        <div className="flex bg-gradient-to-l from-green-950 to-black m-10">
          <section className="relative w-96 h-96 overflow-hidden hidden lg:block">
            {info && info.picture ?
              <Image src={info.picture} width={918} height={1032} alt="Image" className="absolute w-full h-full object-cover"/>
              :
            ...
            <Tabs defaultValue={experiencias[0].title}>
              <TabsList className="w-full bg-neutral-900 text-white">
                ...
                {experiencias.map(experiencia => (
                  experiencia.isMain && <TabsTrigger key={experiencia.id} value={experiencia.id} className='w-full'>{experiencia.title}</TabsTrigger>
                ))}
                </div>
                <div className='lg:hidden block'>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      {experiencias.map(experiencia => (
                        experiencia.isMain && <DropdownMenuItem key={experiencia.id}><TabsTrigger value={experiencia.id} className='w-full'>{experiencia.title}</TabsTrigger></DropdownMenuItem>
                      ))}
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                ...
                experiencia.isMain && (
                  <TabsContent key={experiencia.id} value={experiencia.id}>
                    <Card className='border-0 text-white text-lg bg-neutral-900 md:grid md:grid-cols-3 md:h-[25rem] flex flex-col'>
                      <Image src={experiencia.image} width={918} height={1032} alt={experiencia.title} className="md:col-span-1 object-cover mb-10 md:mb-0 self-center md:justify-self-center rounded-xl w-[90%]"/>
                      <div className='md:col-span-2 md:overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-neutral-900'>
                        ...
                        <CardFooter>
                          <div>
                            {experiencia.footer}
                            <span> </span>
                            {experiencia.links && Object.entries(experiencia.links).map(([key, value]) => (
                              <div key={key}>
                                <Link href={value} className='text-green-400'>{key}</Link>
                              </div>
                            ))}
                          </div> 
                        </CardFooter>
          ...
          <Tabs defaultValue={competencias[0].id} className='lg:flex'>
            <TabsList className="lg:grid lg:grid-rows-5 lg:h-fit text-white bg-neutral-900 gap-3 lg:ml-6 lg:mt-6 w-full lg:w-fit">
              {competencias.map(competencia => (
                <TabsTrigger key={competencia.id} value={competencia.id} className={`hidden lg:block text-${competencia.color}`}>{competencia.title}</TabsTrigger>
              ))}
              <div className='lg:hidden block w-full text-center'>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    {competencias.map(competencia => (
                      <DropdownMenuItem key={competencia.id}><TabsTrigger value={competencia.id} className='w-full'>{competencia.title}</TabsTrigger></DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            ...
            {competencias.map(competencia => (
              <TabsContent key={competencia.id} value={competencia.id}>
                <Card className="border-0 text-white text-lg bg-neutral-900 md:grid md:grid-cols-3 md:h-[25rem] flex flex-col">
                  <div className='md:col-span-2 md:overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-neutral-900'>
                    <CardHeader>
                      <CardTitle className="text-3xl">{competencia.title}</CardTitle>
                      <CardDescription className="text-xl">
                        {competencia.header}
                      </CardDescription>
                    </CardHeader>
                  ...
                  <Image src={competencia.image} height={1000} width={1000} alt='Image from the competition' className='md:col-span-1 object-cover mb-10 md:mb-0 self-center md:justify-self-center rounded-xl w-[90%]'/>
                </Card>
              </TabsContent>
            ))}
          </Tabs>
